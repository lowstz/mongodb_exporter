...
func (group *Group) trackField(fieldName string, value float64) {
	...
	// glog.V(2).Infof("Setting %s(metrics)=%f (%s,%s)", group.Name, value, collectorType, fieldName)
...
func (group *Group) trackFieldsVec(fields GroupFieldsMap, labels []string, value float64) {
	...
	//  glog.V(2).Infof("Setting %s(%s)=%f (%s=%v)", group.Name, metadata.Type, value, metadata.Labels, labels)
